...
# each of the "cover" elements will really be a function that checks if a datapoint lives in it
xcover = apply(create_width_balanced_cover(min(projx), max(projx), num_bins, percent_overlap), 1, check_in_interval)
ycover = apply(create_width_balanced_cover(min(projy), max(projy), num_bins, percent_overlap), 1, check_in_interval)
zcover = apply(create_width_balanced_cover(min(projz), max(projz), num_bins, percent_overlap), 1, check_in_interval)
eccentriccover = apply(create_width_balanced_cover(min(eccentricity), max(eccentricity), num_bins, percent_overlap), 1, check_in_interval)

xmapper = create_mapper_object(data = P.data, dists = P.dist, filtered_data = projx, cover_element_tests = xcover, method = "single")
ymapper = create_mapper_object(P.data, P.dist, projy, ycover, "single")
zmapper = create_mapper_object(P.data, P.dist, projz, zcover, "single")
eccentricmapper = create_mapper_object(P.data, P.dist, eccentricity, eccentriccover, "single")

# mappeR also has a built-in 1D mapper function
xmapper2 = create_1D_mapper_object(P.data, P.dist, projx, num_bins, percent_overlap, "single")
```
...
imappergraphx = imapper(xmapper2)
plot(imappergraphx, xlim = c(0, 2))
legend(
  "right",
  title = "x-coordinate",
...
```{r ballmapper_parameters, echo=TRUE}
