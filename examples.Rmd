...
# cymapper(flare.data, flare.data$x, dist(flare.data), 10, 15, "single")
cyballmapper(flare.data, dist(flare.data), 3)
```

```{r, echo=FALSE}

spiral.data = data.frame( x= sapply((seq(0,6*pi,by=.1), function(x) 3*x*cos(x)) + rnorm(100, 500, .03), y = sapply(1:1000, function(x) x*sin(x)) + rnorm(100, 0, 0.03))

plot(spiral.data$x, spiral.data$y, pch=20)
# cyballmapper(spiral.data, dist(spiral.data), 50)
```
